...
class FastFineTuneConfig:
    ...
    output_dir: str = "models/flan-t5-qg-fast"
...
class FastQuestionGenerationTrainer:
    def __init__(self, config: FastFineTuneConfig):
        self.config = config
        self.tokenizer = None
        self.model = None
    ...
    def load_data(self):
        ...
        val_data = val_data[:self.config.max_val_samples]
    ...
    def train(self):
        ...
        total_steps = len(self.train_dataset) // (self.config.batch_size * self.config.gradient_accumulation_steps)
        print(f"Total training steps: ~{total_steps}")
        print(f"Expected time: ~1-2 hours (with GPU) or ~8-12 hours (CPU)")
        print("="*60 + "\n")
...
def main():
    config = FastFineTuneConfig()

    print("="*60)
    print("FAST Question Generation Fine-Tuning")
    print("="*60)
    ...
    trainer = FastQuestionGenerationTrainer(config)
    ...
    print("2. Use in generator: Update model_name in config")
